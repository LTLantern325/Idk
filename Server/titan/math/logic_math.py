"""
Python conversion of Supercell.Laser.Titan.Math.LogicMath.cs
Mathematical functions for game logic
"""

import math
from typing import Tuple

class LogicMath:
    """Mathematical functions for Brawl Stars game logic"""

    # Precomputed lookup tables for performance
    SQRT_TABLE = bytes([
        
    ])

    ATAN_TABLE = bytes([
        
    ])

    SIN_TABLE = [
        0x0000, 0x0012, 0x0024, 0x0036, 0x0047, 0x0059, 0x006B, 0x007D,
        0x008F, 0x00A0, 0x00B2, 0x00C3, 0x00D5, 0x00E6, 0x00F8, 0x0109,
        0x011A, 0x012B, 0x013C, 0x014D, 0x015E, 0x016F, 0x0180, 0x0190,
        0x01A0, 0x01B1, 0x01C1, 0x01D1, 0x01E1, 0x01F0, 0x0200, 0x020F,
        0x021F, 0x022E, 0x023D, 0x024B, 0x025A, 0x0268, 0x0276, 0x0284,
        0x0292, 0x02A0, 0x02AD, 0x02BA, 0x02C7, 0x02D4, 0x02E1, 0x02ED,
        0x02F9, 0x0305, 0x0310, 0x031C, 0x0327, 0x0332, 0x033C, 0x0347,
        0x0351, 0x035B, 0x0364, 0x036E, 0x0377, 0x0380, 0x0388, 0x0390,
        0x0398, 0x03A0, 0x03A7, 0x03AF, 0x03B5, 0x03BC, 0x03C2, 0x03C8,
        0x03CE, 0x03D3, 0x03D8, 0x03DD, 0x03E2, 0x03E6, 0x03EA, 0x03ED,
        0x03F0, 0x03F3, 0x03F6, 0x03F8, 0x03FA, 0x03FC, 0x03FE, 0x03FF,
        0x03FF, 0x0400, 0x0400
    ]

    @staticmethod
    def abs(value: int) -> int:
        """Absolute value"""
        return abs(value)

    @staticmethod
    def cos(angle: int) -> int:
        """Cosine using lookup table"""
        return LogicMath.sin(angle + 90)

    @staticmethod
    def cos_scaled(angle_a: int, angle_b: int) -> int:
        """Cosine with scaling"""
        return LogicMath.sin_scaled(angle_a + 90, angle_b)

    @staticmethod
    def sin(angle: int) -> int:
        """Sine using lookup table"""
        angle = LogicMath.normalize_angle_360(angle)

        if angle < 180:
            if angle > 90:
                angle = 180 - angle
            return LogicMath.SIN_TABLE[angle]

        angle -= 180
        if angle > 90:
            angle = 180 - angle

        return -LogicMath.SIN_TABLE[angle]

    @staticmethod
    def sin_scaled(angle_a: int, angle_b: int) -> int:
        """Sine with scaling"""
        angle_a = LogicMath.normalize_angle_360(angle_a)

        if angle_a < 180:
            if angle_a > 90:
                angle_a = 180 - angle_a
            return LogicMath.SIN_TABLE[angle_a] * angle_b // 1024

        angle_a -= 180
        if angle_a > 90:
            angle_a = 180 - angle_a

        return -LogicMath.SIN_TABLE[angle_a] * angle_b // 1024

    @staticmethod
    def get_angle(x: int, y: int) -> int:
        """Get angle from coordinates"""
        if x == 0 and y == 0:
            return 0

        if x > 0 and y >= 0:
            if y >= x:
                return 90 - LogicMath.ATAN_TABLE[(x << 7) // y]
            return LogicMath.ATAN_TABLE[(y << 7) // x]

        num = LogicMath.abs(x)

        if x <= 0 and y > 0:
            if num < y:
                return 90 + LogicMath.ATAN_TABLE[(num << 7) // y]
            return 180 - LogicMath.ATAN_TABLE[(y << 7) // num]

        num2 = LogicMath.abs(y)

        if x < 0 and y <= 0:
            if num2 >= num:
                if num2 == 0:
                    return 0
                return 270 - LogicMath.ATAN_TABLE[(num << 7) // num2]
            return 180 + LogicMath.ATAN_TABLE[(num2 << 7) // num]

        if num < num2:
            return 270 + LogicMath.ATAN_TABLE[(num << 7) // num2]

        if num == 0:
            return 0

        return LogicMath.normalize_angle_360(360 - LogicMath.ATAN_TABLE[(num2 << 7) // num])

    @staticmethod
    def get_angle_between(a1: int, a2: int) -> int:
        """Get angle between two angles"""
        result = (a1 - a2) % 360
        if result < 0:
            result += 360

        if result > 179:
            result -= 360

        return abs(result)

    @staticmethod
    def sign(value: int) -> int:
        """Sign function"""
        if value > 0:
            return 1
        elif value < 0:
            return -1
        return 0

    @staticmethod
    def normalize_angle_180(angle: int) -> int:
        """Normalize angle to 0-179"""
        angle = LogicMath.normalize_angle_360(angle)
        if angle >= 180:
            return angle - 180
        return angle

    @staticmethod
    def normalize_angle_360(angle: int) -> int:
        """Normalize angle to 0-359"""
        angle = angle % 360
        if angle < 0:
            angle += 360
        return angle

    @staticmethod
    def sqrt(value: int) -> int:
        """Fast integer square root using lookup table"""
        if value < 0:
            return -1

        if value >= 0x10000:
            if value >= 0x1000000:
                if value >= 0x10000000:
                    if value >= 0x40000000:
                        if value == 0x7FFFFFFF:
                            return 0xFFFF
                        num = LogicMath.SQRT_TABLE[value >> 24] << 8
                    else:
                        num = LogicMath.SQRT_TABLE[value >> 22] << 7
                else:
                    if value >= 0x4000000:
                        num = LogicMath.SQRT_TABLE[value >> 20] << 6
                    else:
                        num = LogicMath.SQRT_TABLE[value >> 18] << 5

                num = (num + 1 + value // num) >> 1
                num = (num + 1 + value // num) >> 1
                return num if num * num <= value else num - 1

            if value >= 0x100000:
                if value >= 0x400000:
                    num = LogicMath.SQRT_TABLE[value >> 16] << 4
                else:
                    num = LogicMath.SQRT_TABLE[value >> 14] << 3
            else:
                if value >= 0x40000:
                    num = LogicMath.SQRT_TABLE[value >> 12] << 2
                else:
                    num = LogicMath.SQRT_TABLE[value >> 10] << 1

            num = (num + 1 + value // num) >> 1
            return num if num * num <= value else num - 1

        if value >= 0x100:
            if value >= 0x1000:
                if value >= 0x4000:
                    num = LogicMath.SQRT_TABLE[value >> 8] + 1
                else:
                    num = (LogicMath.SQRT_TABLE[value >> 6] >> 1) + 1
            else:
                if value >= 0x400:
                    num = (LogicMath.SQRT_TABLE[value >> 4] >> 2) + 1
                else:
                    num = (LogicMath.SQRT_TABLE[value >> 2] >> 3) + 1

            return num if num * num <= value else num - 1

        if value >= 0:
            return LogicMath.SQRT_TABLE[value] >> 4

        return -1

    @staticmethod
    def clamp(clamp_value: int, min_value: int, max_value: int) -> int:
        """Clamp value between min and max"""
        if clamp_value >= max_value:
            return max_value
        if clamp_value <= min_value:
            return min_value
        return clamp_value

    @staticmethod
    def max(value_a: int, value_b: int) -> int:
        """Maximum of two values"""
        return max(value_a, value_b)

    @staticmethod
    def min(value_a: int, value_b: int) -> int:
        """Minimum of two values"""
        return min(value_a, value_b)

    @staticmethod
    def get_radius(x: int, y: int) -> int:
        """Get radius/distance from origin"""
        x = LogicMath.abs(x)
        y = LogicMath.abs(y)
        max_value = LogicMath.max(x, y)
        min_value = LogicMath.min(x, y)
        return max_value + (53 * min_value >> 7)

    @staticmethod
    def get_rotated_x(x: int, y: int, angle: int) -> int:
        """Get X coordinate after rotation"""
        return (x * LogicMath.cos(angle) - y * LogicMath.sin(angle)) // 1024

    @staticmethod
    def get_rotated_y(x: int, y: int, angle: int) -> int:
        """Get Y coordinate after rotation"""
        return (x * LogicMath.sin(angle) + y * LogicMath.cos(angle)) // 1024
